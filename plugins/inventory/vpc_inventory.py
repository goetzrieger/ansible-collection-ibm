    ...
    def get_virtual_server_instances(self):
        ...
        for region in self.regions:
            ...
            if instances and self.use_floating_ips:
                ...
                logger.debug(f"Found {len(fips)} total floating IPs, {len(bound_fips)} are bound to instances in region {region}")
                
                for fip in bound_fips:
                    # Assumption #1: Floating IPs can only target one VSI
                    fip_addr = fip.get("address")
                    
                    # Handle both target as array and target as object
                    ...
                    if isinstance(target, list) and len(target) > 0:
                        ...
                        logger.debug(f"Floating IP {fip_addr} has unexpected target structure: {target}")
                        continue
                        
                    logger.debug(f"Processing floating IP {fip_addr} with target ID {target_id}")

                    for instance in instances:
                        # Assumption #2: VSIs may only have one Floating IP
                        # Fix: primary_network_interface is a list, access the first element's id
                        if (instance.get("primary_network_interface") 
                            and len(instance.get("primary_network_interface")) > 0):
                            interface_id = instance.get("primary_network_interface")[0].get("id")
                            logger.debug(f"Checking instance {instance.get('name', 'unnamed')} with interface ID {interface_id}")
                            if target_id == interface_id:
                                instance['floating_ip'] = fip_addr
                                logger.debug(f"Successfully matched floating IP {fip_addr} to instance {instance.get('name', 'unnamed')}")
                        else:
                            logger.debug(f"Instance {instance.get('name', 'unnamed')} has no primary_network_interface or empty interface list")

        # If using Floating IPs, remove VSIs that don't have one
        if self.use_floating_ips:
            original_count = len(vsis)
            vsis = [v for v in vsis if "floating_ip" in v]
            logger.debug(f"Filtered VSIs: {original_count} total VSIs, {len(vsis)} have floating IPs")
